...
def sample_square_part(
    ...
    :return:
        square image of size `internal_size + 2 * padding_size`
    """
...
def generate_blurry_negative_example(
        part: np.ndarray, padding_size: int, noise_intensity: float = 0.2
) -> np.ndarray:
    """
    ...
    noisy_mask = pad_image(noisy_mask, padding_size)
...
def generate_mismatching_negative_example(
        part: np.ndarray, another_part: np.ndarray, padding_size: int
) -> np.ndarray:
    """
    ...
    another_center, another_frame = split_into_center_and_frame(
        another_part, padding_size
    )
    another_center = pad_image(another_center, padding_size)
...
def yield_discriminator_batches(
        ...
        internal_size: int, padding_size: int
    ...
    for i in range(images.shape[0]):
        ...
        for j in range(positive_batch.shape[0]):
            ...
            another_part = sample_square_part(
                another_image, internal_size, padding_size
            )
            mismatching_part = generate_mismatching_negative_example(
                real_part, another_part, padding_size
            )
            features.append(
